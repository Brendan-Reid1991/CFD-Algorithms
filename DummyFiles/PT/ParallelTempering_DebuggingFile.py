...
spinpath = "spinweights%s.dat"%Dimension
swfile = open(spinpath)
SpinWeights = json.load(swfile)
jpath = "JMat%s.mtx"%Dimension
hpath = "HMat%s.dat"%Dimension
cpath = "const%s.dat"%Dimension
J = mmread(jpath)
H = np.loadtxt(hpath)
C = np.loadtxt(cpath)
...
def ParTemp(data, initial_faults, Replicas, Swaps, TempRange):
    ...
    iop = data[1]
    faults = data[0]
    print("True fault configuration: %s"%np.sort(faults))
    #Dimension of circuit defined
    n = len(iop[0][0])
    ...
    while i<Replicas:
        ...
        i += 1
    print("\nInitial active faults:")
    for i in range(0,Replicas):
        print(np.sort(Active[i]))
    ...
    while i<Replicas:
        ...
        Energies[i] = (cost)
        i+= 1
    print("\nInverse temperatures: %s"%Temperatures)
    print("Initial energies :%s\n"%Energies)
    ...
    # first_gs will record when the first groundstate was found. Will return zero if none found.

    Swap_Intervals = np.linspace(0,Swaps,10,dtype=int)
    while F<=MaxF:
        FailSafe = 0
        if F>initial_faults:
            print("\nIncreasing faults to %s\n    "%F)
            Active = np.zeros((Replicas,F))
            i = 0
            while i<Replicas:
                Active[i] = random.sample(range(2*n,6*n**2-6*n),F)
            ...
            while i<Replicas:
                for element in FaultPositions:
                    if element in Active[i]:
                        ...
                        V[i][element] = -1
                i+=1
            print("New active faults for each replica:")
            for i in range(0,Replicas):
                print("%s"%(np.sort(Active[i])))
            i = 0
            while i<Replicas:
                cost = V[i]@J@V[i]+V[i]@H+C
                Energies[i] = (cost)
        ...
        while FailSafe<FailSafeMax:
            ...
            while swapnum<Swaps:
                ...
                if int(val) == 0:
                    print("Solution found for pair %s. \n   Current active faults: %s"%(p,np.sort(Active[idx])))
                    ...
                    if FailSafe == 0:
                        ...
                        print("\nFirst groundstate for %s faults found at %s%% through the simulation.\n"%(F,first_gs[-1][1]*100))
                    ...
                    if solnflag<len(iop):
                        ...
                        print("Configuration worked for %s pairs. Reinitialising."%solnflag)
                        ...
                        while i<Replicas:
                            Active[i] = random.sample(range(2*n,6*n**2-6*n),F)
                            i+= 1
                        print("Restarting with pair %s of %s"%(p,len(iop)))
                        print("New active faults:")
                        for i in range(0,Replicas):
                            print(Active[i])
                        while M<Replicas:
                            for x in range(0,2*n):
                ...
                while M<Replicas-1:
                    ...
                    M += 1
                if swapnum in Swap_Intervals:
                    print("Swap %s, energies: %s"%(swapnum+1,Energies))
                swapnum+=1
            if swapnum==Swaps:
                break
        if solnflag == len(iop):
            print("Solution found! Configuration given by %s"%(np.sort(Active[M])))
